...
class CdnResourceReuest(BaseContorller):
    ...
    def list(self, resource_ids=None, name=None, etype=None,
        ...
        for resource in resources:
            info = dict(entity=resource.entity,
                        ...
                        impl=resource.impl)
            if versions:
                info.setdefault('versions',
                                sorted(_versions.get(resource.resource_id, []),
    ...
    def index(self, req, body=None):
        ...
        cdndomains = {}
        for cdndomain in query.all():
            cdndomains[cdndomain.entity] = dict(domains=[domain.domain
                                                         for domain in cdndomain.domains],
                                                port=cdndomain.port,
                                                agent_id=cdndomain.agent_id,
                                                internal=cdndomain.internal)
        for r in results['data']:
            r['cdndomain'] = cdndomains[r['entity']]
        return results
...
class CdnQuoteRequest(BaseContorller):
    ...
    def delete(self, req, quote_id, body=None):
        ...
        if not quote:
            LOG.warning('Quote id %d not found, but return success' % quote_id)
